...
python freq_words_extraction.py   \
    ...
    --random_seed 42  \
    --alpha 2.0 \
    --template "[INST] Read the following coded text and track the frequency of each coded word. \
Find the three most frequently appeared coded words. {context}\nQuestion: Do not provide any \
explanation. Please ignore the dots '....'. What are the three most frequently appeared words in \
the above coded text? [/INST] Answer: According to the coded text above, the three most frequently \
appeared words are:"
"""
...
def generate_input_output(
    ...
    def gen_text(num_words: int) -> tuple[str, list[str]]:
        k = np.arange(1, len(vocab) + 1)
        sampled_cnt = num_words * (k**-alpha) / zeta(alpha)
        sampled_words_list: list[str] = [
            w for w, zi in zip(vocab, sampled_cnt.astype(int), strict=False) for _ in range(zi)
        ]
        random.Random(args.random_seed).shuffle(sampled_words_list)
        return template.format(context=" ".join(sampled_words_list), query=""), vocab[1:4]
...
def sys_kwext(num_samples: int, max_seq_length: int, incremental: int = 10) -> list[dict[str, object]]:
    write_jsons = []
...
def main() -> None:
    ...
    write_jsons = sys_kwext(num_samples=args.num_samples, max_seq_length=args.max_seq_length, incremental=10)
