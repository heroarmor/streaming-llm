...
def postprocess_pred(predict_str: str, task_config: dict) -> str:
...
def run_evaluation_per_task(
    task_config: dict, predictions_file: str, verbose: int = 0
    ...
    if verbose != 0:
        print("=" * 40)
        for i, (input, reference, predict) in enumerate(zip(inputs, references, predicts, strict=True)):
            print(f"Input     : {input}")
...
def write_evaluation(results: dict) -> None:
...
def write_submission(results: dict) -> None:
...
def aggregate_chunk(folder: str) -> None:
